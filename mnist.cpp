...
Tensor read_images(const std::string& root, bool train) {
  ...
  std::ifstream images(path.c_str(), std::ios::binary);
...
MyMNIST::MyMNIST(const std::string& root, Mode mode)
    : images_(read_images(root, mode == Mode::kTrain)),
      targets_(read_targets(root, mode == Mode::kTrain)) {}

Example<> MyMNIST::get(size_t index) {
  return {images_[index], targets_[index]};
}

optional<size_t> MyMNIST::size() const {
  return images_.size(0);
}

// NOLINTNEXTLINE(bugprone-exception-escape)
bool MyMNIST::is_train() const noexcept {
  return images_.size(0) == kTrainSize;
}

const Tensor& MyMNIST::images() const {
  return images_;
}

const Tensor& MyMNIST::targets() const {
  return targets_;
}
